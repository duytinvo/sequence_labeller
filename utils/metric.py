...
def recover_label(pred_variable, gold_variable, mask_variable, l2i, word_recover):
    """
        input:
    ...
    for idx in range(batch_size):
        pred = [i2l[pred_tag[idx][idy]] for idy in range(seq_len) if mask[idx][idy] != 0]
        gold = [i2l[gold_tag[idx][idy]] for idy in range(seq_len) if mask[idx][idy] != 0]
